 ...
 * `wrapAction(...)` registrations for them — so bolting verb onto an existing app is a
 ...
 * `agent-native wrap` CLI path pays for it.
 */
...
async function loadTypeScript(): Promise<typeof TS> {
  try {
    ...
    throw new Error(
      "`agent-native wrap` needs the TypeScript compiler. Install it: npm i -D typescript",
    );
  }
}
...
function renderModule(fns: FnInfo[], importPath: string): string {
  ...
  const header = [
    "// AUTO-GENERATED by `agent-native wrap`. Review each description + TODO and the inferred",
