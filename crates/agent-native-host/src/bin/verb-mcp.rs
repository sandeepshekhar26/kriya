//! `verb-mcp` — expose a verb app's actions as a **governed** MCP server over stdio.
//!
...
//! gates before — and only if — it reaches the app. This is the bolt-on that turns verb
...
//!
//! Usage:
//!   verb-mcp --tools <schemas.json> [--policy <policy.yaml>] [--exec "<cmd>"]
...
//!   --name      server name reported in `initialize` (default: verb-mcp)
...
fn build_executor(exec: Option<String>, persistent: bool) -> Box<dyn ActionExecutor> {
    match exec {
        ...
        None => Box::new(FnExecutor(|_id: &str, _p: &serde_json::Value| {
            ActionOutcome::failed("verb-mcp started without --exec — discovery only, cannot run actions")
        })),
    }
}
