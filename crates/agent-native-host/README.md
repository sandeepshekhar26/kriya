...
The Rust agent host behind [**verb**](https://github.com/sandeepshekhar26/verb) — the **governed
...
Pairs with the [`@agent-native/core`](https://www.npmjs.com/package/@agent-native/core)
...
with every `tools/call` routed through the gates. This is what makes verb a bolt-on, not a rewrite:

```rust
use std::sync::Arc;
use agent_native_host::{audit::Signer, permissions::Policy};
use agent_native_host::mcp::{Governor, Server, ProcessExecutor, DenyApproval};
...
Or just use the bundled **`verb-mcp`** binary:
`verb-mcp --tools tools.json --policy agent-policy.yaml --exec "node handler.js" --persistent`.

**2. In-process agent loop** — host an autonomous agent inside your app and drive it through a
[`HostSink`](src/agent/transport.rs) (Tauri, or the `verb-host` stdio sidecar for Electron/Node):
...
- `verb-mcp` — the governed MCP server (external agents).
- `verb-host` — the stdio sidecar (Electron/Node host the runtime via `@agent-native/sidecar`).

## Status

Alpha (`0.1.0`), part of the verb framework. MIT licensed. The host crate ships **no** committed
